...
?>
        ...
        foreach ($search as $key => $value):
            ...
            ${'user' . $key}->showDiscord = $value['showDiscord'];
            ${'user' . $key}->showMail = $value['showMail'];
    ...
    <?php foreach ($userRanks as $secondKey => $secondValue):
        ?><li class="list-group-item bg-dark-opac text-light"><img class="img-fluid" src="../assets/Images/<?= $secondValue['image'] ?>" /><?= 'rang : ' . $secondValue['rank'] ?></li>
                            ...
                            <div id="<?= 'collapse' . $key . '' . $key ?>" class="collapse" aria-labelledby="heading<?= $key . '' . $key ?>" data-parent="#accordion<?= $key ?>">
                                <ul class="list-group list-group-flush bg-light-opac">
                                    <li class="text-center list-group-item bg-dark-opac text-light"><?=(${'user' . $key}->showMail =='Yes')?'<a href="mailto:'. ${'user' . $key}->mail .'">' . ${'user' . $key}->mail . '</a>':'Ce membre ne souhaite pas partager son email' ?></li>
                                    <li class="text-center list-group-item bg-dark-opac text-light"><?=(${'user' . $key}->showDiscord =='Yes')?${'user' . $key}->tagDiscord :'Ce membre ne souhaite pas partager son discord' ?></li>
                                </ul>
                                <div class="card-body">
